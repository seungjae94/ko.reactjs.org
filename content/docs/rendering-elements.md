...
React 엘리먼트를 렌더링하기 위해서는 먼저 [`ReactDOM.createRoot()`](/docs/react-dom-client.html#createroot)에 루트 DOM 노드를 인자로 넘기고, 다음으로 `root.render()`에 React 엘리먼트를 인자로 넘겨야 합니다.
...
위 함수는 [`setInterval()`](https://developer.mozilla.org/en-US/docs/Web/API/WindowTimers/setInterval) 콜백을 이용해 초마다 `root.render()`를 호출합니다.

>**주의**
>
>실제로 대부분의 React 앱은 `root.render()`를 한 번만 호출합니다. 다음 장에서는 이와 같은 코드가 [유상태 컴포넌트](/docs/state-and-lifecycle.html)에 어떻게 캡슐화되는지 설명합니다.
>
